...
func JWTWXMPAuth() gin.HandlerFunc {
    return func(c *gin.Context) {
        ...
        if jwtWxmpService.IsBlacklist(token) {
        ...
        //	_ = jwtWxmpService.JsonInBlacklist(system.JwtBlacklist{Jwt: token})
        //	response.FailWithDetailed(gin.H{"reload": true}, err.Error(), c)
        //	c.Abort()
        //}
        if claims.ExpiresAt-time.Now().Unix() < claims.BufferTime {
            ...
            if global.GVA_CONFIG.System.UseMultipoint {
                err, RedisJwtToken := jwtWxmpService.GetRedisJWT(newClaims.Username)
                if err != nil {
                    global.GVA_LOG.Error("get redis jwt failed", zap.Any("err", err))
                } else { // 当之前的取成功时才进行拉黑操作
                    _ = jwtWxmpService.JsonInBlacklist(system.JwtBlacklist{Jwt: RedisJwtToken})
                }
                // 无论如何都要记录当前的活跃状态
                _ = jwtWxmpService.SetRedisJWT(newToken, newClaims.Username)
            }
        }
        c.Set("claims", claims)
        c.Next()
